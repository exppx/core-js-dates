...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  return Math.ceil(
    (new Date(dateEnd) - new Date(dateStart) + 1) / 1000 / 3600 / 24
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  while (d.getMonth() === month - 1) {
    if (d.getDay() === 6 || d.getDay() === 0) weekends += 1;
    d.setDate(d.getDate() + 1);
  }
...
function getWeekNumberByDate(date) {
  const d = new Date(date.getFullYear(), 0);
  let days = 0;
...
function getQuarter(date) {
  ...
  const yearDays = date.getFullYear() % 4 === 0 ? 366 : 365;
  const part = days / yearDays;

  if (part <= 0.25) return 1;
  if (part <= 0.5) return 2;
  if (part <= 0.75) return 3;
  return 4;
}
